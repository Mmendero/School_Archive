...
<html>
    ...
    <?php
      ...
      if(count($all_inputs) == 0){
        echo '<p class = "results-txt">No inputs made. :(</p>';
        exit;
      }
      ...
      echo '<p class = "results-txt">'.'Query: '.$query.'</p>';
      ...
      for ($i=0; $i <$num_results; $i++) {
        ...
        echo '</p>';
        echo '';
        echo '<input type="hidden" name="productid" value="'.stripslashes($row['id']).'">';
        echo '<div class="input-group-append">';
        echo '<button class="btn btn-primary" name="delete_entry" type="submit">Delete Entry</button>';
        echo '</div>';
        echo '</form>';
        echo '</div>';
        echo '</div>';
